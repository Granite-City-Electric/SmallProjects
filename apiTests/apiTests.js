...
const getTeamData = async (id) => {
    const res1 = await fetch(`https://sports.core.api.espn.com/v2/sports/football/leagues/nfl/seasons/2024/types/2/teams/${id}/record`);
...
// Run Comparisons post API calls
...
//     getTeamData(team.id);
//   })
//   contestantsThisYear.forEach(contestant => calcRecord(contestant));
...
// write function that takes in the team list, makes the api calls and returns a new team list to a variable. pass that team list into the function that calculates the user records (so need to pass the picks into function as well). then pass those standings into function that makes table.

// const getTeamData2 = async (team) => {
